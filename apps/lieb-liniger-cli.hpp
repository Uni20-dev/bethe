...
namespace bethe::cli::lieb_liniger_app
{
namespace model = bethe::lieb_liniger;
namespace cli = bethe::cli;
template <bool HardWalls, uni20::Real Real>
using State = std::conditional_t<HardWalls, model::open::State<Real>, model::State<Real>>;
struct Arguments
{
...
template <bool HardWalls> auto program_info()
{
  auto info = bethe::cli::program_info(
      HardWalls ? "bethe-lieb-liniger-obc" : "bethe-lieb-liniger-pbc",
      HardWalls ? "Repulsive continuum bosons in a hard-wall box: N>=0, finite ELL>0 and C>0."
                : "Repulsive continuum bosons on a ring: N>=0, finite ELL>0 and C>0.",
      HardWalls ? bethe::citations::Tool::lieb_liniger_obc : bethe::citations::Tool::lieb_liniger_pbc);
  info.notes = {"H=-sum_j d_j^2 + 2c sum_(i<j) delta(x_i-x_j), hbar^2/(2m)=1.",
                ...
                HardWalls ? "Positive k are standing-wave Bethe coordinates, not a conserved total momentum."
                          : "Momentum P=2*pi*sum(I)/ELL is signed, with no Brillouin-zone reduction.",
                "Attraction, c=0, c=infinity, and thermodynamics are not implemented.",
                HardWalls ? "Dirichlet walls only. See docs/lieb-liniger-open.md for equations and numerical limits."
...
template <bool HardWalls> void add_options(CLI::App& app, Arguments& args)
{
  bethe::cli::option(app, "N", args.particles, "Number of bosons")->required();
  bethe::cli::option(app, "--length", args.length, HardWalls ? "required box length" : "required ring circumference")
      ->required();
  ...
  bethe::cli::option(app, "--quantum-numbers", args.numbers,
                     HardWalls ? "strictly increasing positive integer labels (default: 1,...,N); none for vacuum"
                               : "explicit sorted labels (default: ground state) integers for odd N, half-odd integers "
                                 "for even N; none for vacuum");
  ...
  bethe::cli::option(
      ...
      HardWalls ? "REQUIRED for scans: P extra upper slots; choose N labels from 1,...,N+P; P=0 is ground only"
  ...
  bethe::cli::option(app, "--roots", args.roots,
                     HardWalls ? "print positive Bethe wave numbers k and integer labels I"
                               : "print physical momenta k and labels I");
...
inline char const* status(model::SolveStatus value)
{
  switch (value)
  {
...
template <bool HardWalls, uni20::Real Real>
void add_state(cli::RunReport& report, State<HardWalls, Real> const& state, std::string id_prefix = "",
               std::string prefix = "")
{
  if (id_prefix.empty())
  ...
  if constexpr (!HardWalls)
    report.field(id_prefix + "momentum_index", prefix + "Momentum index", state.momentum_index)
        .field(id_prefix + "momentum", prefix + "Momentum", state.momentum);
  report.field(id_prefix + "residual_norm", prefix + "Residual norm", state.residual_norm)
      .field(id_prefix + "iterations", prefix + "Iterations", state.iterations);
}

template <bool HardWalls, uni20::Real Real>
void write_output(cli::RunReport& report, Arguments const& args,
                  std::vector<State<HardWalls, Real> const*> const& states,
                  std::vector<std::optional<Real>> const& gaps, State<HardWalls, Real> const* reference = nullptr,
                  State<HardWalls, Real> const* failed = nullptr)
{
  ...
  auto write_states = [&](std::string name, std::string title, auto const& rows, std::size_t offset, bool ranked) {
      ...
      output.table(
          ...
          [&](auto& table) {
            for (std::size_t i = 0; i < rows.size(); ++i)
            {
              auto const& s = *rows[i];
              if constexpr (HardWalls)
                table.append(offset + i, s.energy, ranked ? gaps[i] : std::nullopt, s.residual_norm, s.iterations,
                             s.converged, std::string(status(s.status)));
              else
          ...
          cli::column<std::optional<Real>>("gap", "Gap"), momentum_columns...,
    ...
    if constexpr (HardWalls)
      ...
      emit(cli::column<std::int64_t>("momentum_index", "Q"), cli::column<Real>("p", "P"));
  };
  ...
  if (args.roots)
    output.table(
        ...
        : args.particles ? "Physical momenta"
                         : "Physical momenta (vacuum; no roots)",
        [&](auto& table) {
...
template <bool HardWalls, uni20::Real Real> int run(Arguments const& args, int argc, char** argv)
{
  uni20::run_context context(program_info<HardWalls>(), {.invocation = std::vector<std::string>(argv, argv + argc)});
  ...
  cli::RunReport report(context, HardWalls ? "Lieb-Liniger (hard walls)" : "Lieb-Liniger (periodic)");
  report.field("particles", "Particles", args.particles)
      ...
      .field("momentum_convention", "Momentum convention",
             HardWalls ? "no conserved total momentum; positive k are standing-wave Bethe coordinates"
                       : "P=2*pi*sum(I)/length; no modular reduction");
  if constexpr (HardWalls)
    report.field("boundaries", "Boundaries", "Dirichlet walls at x=0 and x=length")
        .field("label_convention", "Labels", "strictly increasing positive integers; ground I=1,...,N");
  bool converged;
  if (args.count)
  {
    ...
    auto const scan = [&] {
      if constexpr (HardWalls)
        return model::open::real_excitations(args.particles, length, c, *args.padding, enumeration, options);
      else
        return model::real_excitations(args.particles, length, c, *args.padding, enumeration, options);
    }();
    computation.finish();
    converged = scan.converged();
    report.field("calculation", "Calculation", "finite-window excitations (including ground state)")
        .field(HardWalls ? "padding_upper_edge" : "padding_per_edge",
               HardWalls ? "Padding at upper edge" : "Padding per edge", *args.padding)
    ...
    add_state<HardWalls, Real>(report, scan.ground_state, "ground_", "Ground ");
    if (scan.first_unconverged)
      add_state<HardWalls, Real>(report, *scan.first_unconverged, "first_failed_", "First failed ");
    ...
    std::vector<State<HardWalls, Real> const*> states;
    ...
    write_output<HardWalls, Real>(report, args, states, gaps, &scan.ground_state,
  ...
  else
  {
    ...
    auto const state = [&] {
      if constexpr (HardWalls)
        return numbers ? model::open::solve_real(length, c, *numbers, options)
                       : model::open::ground_state(args.particles, length, c, options);
      else
        return numbers ? model::solve_real(length, c, *numbers, options)
    ...
    add_state<HardWalls, Real>(report, state);
    report.status(converged ? cli::semantic_glyph::success : cli::semantic_glyph::warning,
                  converged ? "converged" : "unconverged estimate");
    write_output<HardWalls, Real>(report, args, {&state}, {std::nullopt});
  }

  if (!converged) std::cerr << "Lieb-Liniger solve incomplete; consider a larger budget or higher precision.\n";
  return converged ? 0 : 2;
}
template <bool HardWalls> int main(int argc, char** argv)
{
  Arguments args;
  return bethe::cli::program_main(
      argc, argv, program_info<HardWalls>(), [&](auto& app) { add_options<HardWalls>(app, args); },
      [&](auto&) {
        validate(args);
        return bethe::cli::dispatch_precision(args.precision,
                                              [&]<uni20::Real Real> { return run<HardWalls, Real>(args, argc, argv); });
      });
}
} // namespace bethe::cli::lieb_liniger_app

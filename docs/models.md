...
| `hubbard` | One-band Hubbard, hopping t=1 | Implemented (limited): [PBC](hubbard.md) and [free-end](hubbard-open.md) ground states; [half-filled](hubbard-dispersion.md) and [doped](hubbard-doped.md) thermodynamic spinon/charge lines, [two-spinon continuum edges](hubbard-continuum.md) and a [charge-continuum library](hubbard-charge-continuum-design.md), for U>0, zero field | Charge-continuum CLI, doped continua, finite-field/string dispersions; finite-size excitations; remaining PBC shell branches and odd rings |
...
- **Hubbard excitations and missing ring sectors:** start with explicitly
  ...
  now resolves three additional two-particle families; CLI exposure remains
  a follow-up. The elementary holon is not automatically a sector minimum.

...
| `lee-yang` | Massive non-unitary M(2,5) on a periodic circle, [Bajnok–el Deeb–Pearce](../CITATIONS.md#bajnok-el-deeb-pearce-2015) | [Native source-free ground-state TBA library/frontend](lee-yang.md) implemented with independent oracle checks. Bulk-subtracted energy and c_eff=2/5 must be distinguished from c=-22/5. A development-only spin-zero one-particle oracle covers 5<=mL<=30; native excited states and UV source continuation remain open |

...
The separate periodic XXZ solver covers ground states and sector minima for
`0 <= Delta <= 1`; see [XXZ chains](#xxz-chains).
...
`--sz`, `--sectors`, precision, tolerance, update budgets, CPU time, roots,
and plain/pretty formatting work as in the XXX programs. Default precision is
fp64. `--sz` and `--sectors` are mutually exclusive. With neither, even N uses

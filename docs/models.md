...
| `xyz` | Zero-field spin-1/2 XYZ chain | [Even-chain ground branch](xyz.md); [thermodynamic spinons, bound branches and two-spinon envelopes](xyz-dispersion.md) | Finite-ring excited spectrum remains |
| `potts` | Critical ferromagnetic three-state clock chain | [Periodic vacuum and charged one-hole levels](potts.md), momentum/Z₃ resolved | Selected family only; neutral excitations and full-spectrum enumeration remain |
...
| `kondo` | The integrable continuum single-impurity Kondo problem | [Native zero-temperature scaling-limit response library and frontend](kondo.md) | First slice complete; no finite-band absolute-energy claim |
| `sine-gordon` | Integrable quantum field theory, finite-volume ground energy via nonlinear integral equations | [Bulk-subtracted vacuum library and frontend implemented](sine-gordon.md) | Excited states remain |
...
- **Wider XXZ anisotropy:** the [periodic ground-state/sector path](xxz.md#easy-axis-ground-states-delta1)
  now includes $`\Delta \gt 1`$, retaining a continuous atan2 scattering phase in
  ...
  $`-1\lt \Delta \lt 0`$ sector ground roots for even rings and open chains, with
...
- **Hubbard excitations and missing ring sectors:** start with explicitly
  ...
  A [native charge-containing continuum library](hubbard-charge-continuum-design.md)
  now resolves three additional two-particle families, exposed through the
  same frontend's `--channel` option. The elementary holon is not automatically
  a sector minimum.
...
Complex branches, other roots of unity, and explicit spin-basis Jordan vectors
...
| `xyz` | Zero-field spin-1/2 XYZ / eight-vertex family, [Baxter](../CITATIONS.md#baxter-1973), [Zhang–Klümper–Popkov](../CITATIONS.md#zhang-klumper-popkov-2024), [Johnson–Krinsky–McCoy](../CITATIONS.md#johnson-krinsky-mccoy-1973) | [Even-chain ground solver](xyz.md) and [thermodynamic spinons, all strictly existing bound indices and two-spinon envelopes](xyz-dispersion.md) for the rectangular theta family. Native precision, explicit bound parity/momentum maps, XY/XXZ limits and small-chain checks. Finite-ring excited enumeration, singular states and spectral weights remain; Sz is not generally conserved |
| `potts` | Critical ferromagnetic three-state Potts chain; [Dasmahapatra et al.](../CITATIONS.md#dasmahapatra-kedem-mccoy-melzer-1994), [Nichols](../CITATIONS.md#nichols-2006), [Fukai et al.](../CITATIONS.md#fukai-kleinemuhl-pozsgay-vernier-2024) | [Periodic vacuum and charged one-hole family](potts.md), native precision, explicit physical momentum/Z₃ charge, finite-ring gaps and scaled dimensions. Reuses the XXZ iteration with audited label selection, not generic auxiliary-state enumeration. Neutral excitations, other charged families, off-critical/chiral couplings, other boundaries and spectral weights remain |
| `haldane-shastry` | Spin-1/2 inverse-chord-square exchange on a ring, [Haldane](../CITATIONS.md#haldane-1988) and [Shastry](../CITATIONS.md#shastry-1988) | [Implemented](haldane-shastry.md): exact motif energies, momenta, Yangian dimensions, spin-sector minima and budgeted SU(2) decomposition with `--spin-content` exports. Full motif scans have an explicit budget; wavefunctions and correlations remain future work |
...
| `kondo` | Integrable continuum single-impurity Kondo Hamiltonian, [Andrei](../CITATIONS.md#andrei-1980), response conventions in [Barcza et al.](../CITATIONS.md#barcza-2020) | [Native response library, frontend and independent oracle](kondo.md): zero-temperature, spin-1/2 single-channel scaling-limit impurity energy change and magnetization in a uniform field. Finite temperature and excited states remain. Not a generic finite-band Anderson impurity or Kondo lattice |
| `sine-gordon` | Continuum sine-Gordon theory, [Destri–de Vega](../CITATIONS.md#destri-de-vega-1992), [Rutkevich](../CITATIONS.md#rutkevich-2020) | [Native complex kernel and finite-volume vacuum library/frontend](sine-gordon.md) implemented; excited states remain. Soliton-mass/coupling and bulk-subtraction conventions specified. Large-volume Bethe–Yang quantization alone omits finite-volume corrections |
| `lee-yang` | Massive non-unitary M(2,5) on a periodic circle, [Bajnok–el Deeb–Pearce](../CITATIONS.md#bajnok-el-deeb-pearce-2015) | [Native ground-state and regular one-particle TBA libraries/frontends](lee-yang.md) implemented with independent oracle checks. Bulk-subtracted energy and c_eff=2/5 must be distinguished from c=-22/5. One-particle levels/gaps cover 5<=mL<=30; UV source continuation remains open |

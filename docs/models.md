...
| `temperley-lieb` | TL singlet-projector chains; spin-1 pure biquadratic model | Implemented (limited): [even free ends](biquadratic.md), AF ground state, module minima, real-root excitations, Q-system searches and targeted two-string singlet; [ferro one-defect band](biquadratic-ferromagnetic.md), [bound pairs](biquadratic-bound-pairs.md), [three-defect droplets](biquadratic-bound-triples.md), [real-root scattering windows](biquadratic-scattering.md) and [mixed pair-plus-defect scans](biquadratic-pair-defect.md) (odd/even), sign-aware Q-system levels; representation multiplicities, [physical-spin content API](biquadratic-spin-content.md), and generic lambda>2 TL API | More general mixed strings and larger droplets, other complex-root families, singlet ranks, odd-chain AF spinon branch, spin-content CLI output, PBC twists, other representations |
...
TL multiplicity spaces is available in the shared library, separately from
auxiliary XXZ spin. **Next slice:** optional spin-content CLI tables,

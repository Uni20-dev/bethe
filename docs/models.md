...
| `q-boson` | Deformed boson hopping on a lattice | Implemented (limited): [fixed-N PBC states](q-boson.md), eta>=0, ground-state CLI and excited-state library, free and phase limits | Excited-state CLI, open boundaries, thermodynamics |
...
| `q-boson` | Integrable q-boson hopping/phase model, PBC at fixed particle number | [Ground-state frontend and excited-state library implemented](q-boson.md); excitation CLI next | Excited-state library complete |
...
| `q-boson` | Deformed boson hopping and its phase-model limit, [Bogoliubov–Izergin–Kitanine](../CITATIONS.md#bogoliubov-1997) and [Pozsgay](../CITATIONS.md#pozsgay-2014-q-boson) | [Fixed-N PBC ground-state frontend and excited-state library implemented](q-boson.md), with small-sector full-spectrum checks, free/phase limits and continuum scaling; excitation CLI remains. Not the standard Bose–Hubbard chain |

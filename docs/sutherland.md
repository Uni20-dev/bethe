...
With `hbar=2m=1`, circumference `L>0` and `N>=1`, our convention is

```text
H = -sum_i d_i^2
    + 2*lambda*(lambda-1)*(pi/L)^2 sum_{i<j} 1/sin^2(pi*(x_i-x_j)/L).
```

The input `lambda>=0` selects the bosonic Jastrow domain, with wavefunctions
behaving as `|x_i-x_j|^lambda` at a collision. It is **not** the raw potential
...
```text
k_j = q * [n_j + lambda*(j-(N-1)/2)],  j=0,...,N-1
E = sum_j k_j^2
momentum_index = sum_j n_j
P = q * momentum_index
E0 = (pi/L)^2 * lambda^2 * N*(N^2-1)/3.
```

Continuum momentum is **not reduced modulo `2*pi`**. The `k_j` are
...
A uniform integer boost `n_j -> n_j+b` changes `P` by `N*q*b` and `E` by
...
```text
gap = q^2 * [sum_j n_j^2
             + lambda * sum_{j=1}^{N-1} j*(N-j)*(n_j-n_(j-1))].
```
...
`-W<=n_j<=W`, including the ground state. `--levels all` returns every ordered

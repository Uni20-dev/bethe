...
| `q-boson` | Deformed boson hopping on a lattice | Implemented (limited): [fixed-N PBC ground states](q-boson.md), eta>=0, free and phase limits, shifted energy and optional roots | Excited states, full spectrum, open boundaries, thermodynamics |
...
| `q-boson` | Integrable q-boson hopping/phase model, PBC at fixed particle number | [Ground-state library/frontend implemented](q-boson.md); excitations next | First slice complete |
...
| `q-boson` | Deformed boson hopping and its phase-model limit, [Bogoliubov–Izergin–Kitanine](../CITATIONS.md#bogoliubov-1997) and [Pozsgay](../CITATIONS.md#pozsgay-2014-q-boson) | [Fixed-N PBC ground-state library/frontend implemented](q-boson.md), with occupation-basis checks, free/phase limits and continuum scaling; excited-state labels remain. Not the standard Bose–Hubbard chain |

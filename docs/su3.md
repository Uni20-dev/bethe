...
exchanges them: `P|a,b> = |b,a>`. We solve the antiferromagnetic ring

```text
H = sum_(j=1)^L P_(j,j+1),     P_(L,L+1) = P_(L,1),     J=1.
```
...
`(L-M1, M1-M2, M2)`. The balanced singlet therefore has

```text
M1 = 2L/3,      M2 = L/3.
I_j = j - (M1-1)/2,    j=0,...,M1-1,
J_a = a - (M2-1)/2,    a=0,...,M2-1.
```
...
Use conventional rapidities lambda and mu and `theta_n(x)=2 atan(2x/n)`:

```text
L theta_1(lambda_j)
  - sum_(k!=j) theta_2(lambda_j-lambda_k)
  + sum_a theta_1(lambda_j-mu_a) = 2 pi I_j,

sum_j theta_1(mu_a-lambda_j)
  - sum_(b!=a) theta_2(mu_a-mu_b) = 2 pi J_a.

E = L - sum_j 1/(lambda_j^2 + 1/4),
P = sum_j [pi - theta_1(lambda_j)]  (mod 2 pi).
```
...
Its Hamiltonian is `sum(P-1)/2`, so **`E_here=2*E_paper+L`**.
...
```text
x_a(q) = (3/pi) atanh[tan(pi*q/L) tan(pi*a/6)].
```
...
polynomial is `E*(E+2)^2*(E^2+2E-12)`, giving the singlet minimum

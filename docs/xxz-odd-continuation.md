...
```math
E_{\mathrm{sea}}(\Delta)=cR+\Delta\left[\frac N4-M+\frac{M^2-R^2}{N}\right].
```
...
```math
E\le\min(E_{\mathrm{sea}},E_{\mathrm{helix}})
+256\epsilon\left[1+N+|E|+|\min(E_{\mathrm{sea}},E_{\mathrm{helix}})|\right].
```
...
```math
128\epsilon\max(N,|\mathrm{minimum\_energy}|).
```

...
```text
H = -sum_j d^2/dx_j^2 + 2c sum_(i<j) delta(x_i-x_j),    c > 0,
E = sum_j k_j^2,                  P = sum_j k_j.
```
...
```text
ell*k_j + 2 sum_(l != j) atan((k_j-k_l)/c) = 2*pi*I_j.
```

Labels are distinct and increasing: integers for odd `N`, half-odd integers
for even `N`. Ground-state labels are `I_j=j-(N-1)/2`, with zero-based `j`.
...
For `g<1` we instead use the algebraically equivalent ordered-root equation

```text
q_j - 2*pi*J_j - 2 sum_(l != j) atan(g/(q_j-q_l)) = 0,
J_j = I_j - [j-(N-1)/2].
```

The integer subtraction in `J_j` is performed before conversion to `Real`.
...
momenta `2*pi*J_j`; it crosses over to `2*pi*I_j` at large `g`.

The reported residual is `max_j |F_j|/s_j`, with
`s_j=max(min(1,sqrt(g)), |q_j|, |target_j|)` and `target_j=2*pi*J_j`
for `g<1`, otherwise `2*pi*I_j`. The vacuum's norm is zero. This measures the
...
`E -> pi^2*N*(N^2-1)/(3*ell^2)`, boosts, reflection, length scaling, Jacobian

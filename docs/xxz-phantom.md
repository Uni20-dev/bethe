...
Write $`\Delta =\cos (\gamma)`$ with $`0\lt \gamma \lt \pi`$. Suppose M roots consist of
...
The function supports $`0\lt p \lt M`$ and $`-1\lt \Delta \le 0`$; the all-phantom construction

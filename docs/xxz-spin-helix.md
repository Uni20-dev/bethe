...
In the sector with `M=N/2-Sz` down spins, the unnormalized amplitude is

```text
psi(j_1,...,j_M) = exp(i*k*(j_1+...+j_M)),  0 <= j_1 < ... < j_M < N.
```
...
`cos(k)-Delta_w=0` to `(H-E_w)psi`. Thus every magnetization projection has
...
For `-1<Delta_w<=0`, the helix has a repeated scaled root

```text
z_w = cot(k/2) = sin(k)/(1-cos(k)).
```

These are infinite conventional rapidities. In affine coordinates
`z=o+s*x`, its monic polynomial is `(x-(z_w-o)/s)^M`. The internal
...
```text
eigenvector_defect_bound = N*abs(Delta_input-Delta_w)/4.
```
...
norm is at most `N*abs(Delta_input-Delta_w)/4`. This bound refers to the
